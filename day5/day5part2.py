...
def getHorLineCords(cords: list) -> list:
    cords2 = list()
    for cord in cords:
        x1,y1 = cord[0].split(",")
        x2,y2 = cord[1].split(",")
...
def getVerLineCords(cords: list) -> list:
    cords2 = list()
    for cord in cords:
        x1,y1 = cord[0].split(",")
        x2,y2 = cord[1].split(",")
...
def getDiaLineCords(cords:list) -> list:
    cords2 = list()
    for cord in cords:
        x1,y1 = cord[0].split(",")
        x2,y2 = cord[1].split(",")
...
def addDiaLines(matrix, cords):
    dia = getDiaLineCords(cords)
    for cord in dia:
        x1 = cord[0]
        y1 = cord[1]
        x2 = cord[2]
        y2 = cord[3]
        if x1 < x2 and y1 < y2: #>v
...
print(addDiaLines(markMatrix(x,cords),cords))
